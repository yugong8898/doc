...
结算处理模块的核心编辑页，用于对单条运单的结算信息进行调整和提交。支持查看（read）和编辑（edit）两种模式，集成了费用明细编辑、风控预警、亏吨货损确认、抹零处理等完整结算业务流程。

结算信息区块已抽离为独立的 `SettlementInfo.vue` 组件，`PaymentMethodPanel` 亦已迁入 `SettlementInfo` 内部；`edit.vue` 通过 props + 事件与 `SettlementInfo` 通信。
...
├── read.vue                        （查看页，详见 waybillSettlement-read.md）
├── index.vue                       （结算列表页）
└── components/
    ├── SettlementInfo.vue          结算信息区块组件（含 PaymentMethodPanel）
...
`formModel.unitRice`（单价）或 `formModel.settlementQuantity`（结算数量）变化时，通过 `watch` 自动重算 `transportCost`，同时向 `CostMessageDetails` 传入新运费 `newTransportCost`：

```
transportCost = unitRice × settlementQuantity（取整方式由 roundingMode 决定）
newTransportCost → CostMessageDetails prop → 触发运费费项联动更新
```

`once` 标志位控制首次渲染不触发计算。特殊逻辑：POW 开头订单且 supplyType === '300' 时，`once` 为 true，不自动重算。

`updBusCostInput(tip)` 由 `SettlementInfo` 组件通过 `@input-change` 事件触发，合并了原来的单价/结算数量/结算重量变化处理。

### 3. 与 SettlementInfo 组件的通信

结算信息区块已抽离为 `SettlementInfo.vue`，`edit.vue` 与它通过以下方式通信：

**Props 传入（父 → 子）**：

| Prop | 说明 |
|------|------|
| `formModel` | 表单数据（双向联动，子组件 v-model 修改会反映到父） |
| `detail` | 运单详情 |
| `dictionary` | 枚举字典 |
| `bindCardInfo` | 绑卡信息（isBindCard / payeePhone） |
| `newCompanyOffLineDatas` | 结算主体下拉选项 |
| `splitMainInfo / splitInfo` | 委托转账主体/规则展示文本 |
| `btnLoading` | 按钮加载状态 |
| `waybillId` | 运单号（`$route.query.waybillId`） |
| `showEditActions` | 是否展示编辑操作（edit.vue 传 `true`） |
| `showDownBtn` | 是否展示下载按钮 |

**事件监听（子 → 父）**：

| 事件 | 触发时机 | 父组件处理 |
|------|---------|-----------|
| `@input-change` | 单价/结算数量/结算重量变化 | `updBusCostInput(tip)` |
| `@open-edit-load-weight` | 点击"修改重量"按钮 | `handleOpenEditLoadWeight()` |
| `@open-confirm-loss` | 点击亏吨确认按钮 | `handleTodialog()` |
| `@load-more` | 结算主体下拉触底加载更多 | `loadMore()` |
...
> 注：原 `validatePaymentEquation`（付款等式校验）和 `validateReceiptPayRatio`（回单付比例校验）已迁移至 `SettlementInfo.vue` 内部，`edit.vue` 不再直接调用。
...
风控弹窗回调（通过 `provide` 注入供子组件调用）：
- `handleCloseRiskDialog`：关闭弹窗，不做任何操作
- `handleSubmitRiskDialog`：判断 allowPass，允许则调 `postSubmitAfterRiskPass`
- `updateRiskWarning`：更新页面风控展示信息（page 类型更新 `highMidWaybillListPage`，dialog-single 类型处理 `highMidWaybillList`）
...
| 组件 | 路径 | 作用 |
|------|------|------|
| `SettlementInfo` | `./components/SettlementInfo` | 结算信息区块（含付款方式、亏涨吨、修改重量入口等） |
...
| `getTrackTimes` | `@/mixins/getTrackTimes.js` | 获取跟踪时间，`handleGetTrack` 通过 provide 注入供子组件调用 |
...
> 单价、结算数量、结算重量、装卸重量、结算主体、付款方式等字段的展示/编辑逻辑已全部收敛进 `SettlementInfo.vue` 组件，`edit.vue` 通过 `formModel` 双向绑定获取最新值。

| 字段 | 说明 | 是否可编辑 |
|------|------|-----------|
| `unitRice` | 单价 | SettlementInfo 内 edit 模式可编辑 |
| `settlementQuantity` | 结算数量 | 非柜/车类型可编辑 |
| `settlementWeight` | 结算重量(吨) | 非吨结算时展示，可编辑 |
| `transportCost` | 运费(元) | 自动计算，不可编辑 |
| `loadWeight / loadRoughGoodsWeight` | 装货净重/毛重 | 不可直接编辑，通过 editLoadWeight 弹窗修改 |
| `unloadWeight / unloadRoughGoodsWeight` | 卸货净重/毛重 | 不可直接编辑，通过 editLoadWeight 弹窗修改 |
...
| `receiptPayment` | 回单付 | SettlementInfo 内 PaymentMethodPanel 可编辑 |
| `cashOnDelivery` | 到付 | 联动计算，SettlementInfo 内展示 |
| `kuiTonsSituationVO` | 亏涨吨情况 | 只读，来自 detail，getDetails 时同步到 formModel |
| `lsdsGoodsDeductible` | 亏涨吨货损抵扣 | 只读，来自 detail，getDetails 时同步到 formModel |
...
1. **SettlementInfo 职责边界**：结算信息区块（单价/数量/付款方式/亏涨吨/修改重量入口）全部在 `SettlementInfo.vue` 内管理，`edit.vue` 只负责接收事件后调用对应方法，不直接操作这些字段的 DOM
2. **versionCode**：修改重量时必须传，用于乐观锁并发控制，从 `getDetails` 返回值中获取并缓存在 `this.versionCode`
3. **once 标志位**：防止 `watch` 在初始化时误触发运费重算，`updBusCostInput` 调用后置为 false；POW 开头且 supplyType=300 的运单始终为 true
4. **newTransportCost**：`updBusCostInput` 计算后赋值给 `this.newTransportCost`，通过 prop 传入 `CostMessageDetails`，驱动运费费项的确认金额联动更新
5. **抹零差价**：`diffAmount` 由 `processNumber` 计算，前端自动追加费项（orderNumber 101/102）并弹窗二次确认；抹零后使用当前用户确认的差价率重新计算，不会覆盖用户已调整的值
6. **风控测算时机**：保存和提交都会先调保存接口、再进行风控测算，风控通过后才真正调用提交接口
7. **货损弹窗**：flag = '1' 时进页面自动弹出，保存/提交前校验 `updateLoseFlag` 是否已确认；flag = '2' 时用户可手动再次打开；运单不存在亏吨时弹提示"亏吨货损变为0"
8. **isInMicroApp**：微应用内查看运单详情/跟踪信息使用 `gotoWaybillDetail`，否则 `window.open` 新标签页跳转
