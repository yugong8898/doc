...
```
ElImageX/
├── index.js              # 注册入口，全局注册为 ElImageX
├── src/
│   ├── main.vue          # 外层容器组件（ElImage），负责图片加载、懒加载、点击触发预览
│   └── image-viewer.vue  # 预览弹窗组件，负责全屏展示、缩放、旋转、下载、保存旋转方向
```

基于 Element UI `el-image` 二次封装，在原有图片展示基础上扩展了：预览导航事件、自定义插槽、图片旋转方向持久化（接口保存）、blob URL 下载支持、下载按钮（可控显示）。

**架构关系**

```
main.vue (ElImage)
  └── image-viewer.vue (elImageViewer) — 通过 v-if="showViewer" 条件渲染
        ├── 使用 PopupManager 管理 z-index 层叠
        ├── 调用 imageRotateSave API（记住旋转方向）
        └── 调用 axios 下载图片（非 blob） / <a> 标签直接下载（blob URL）
```

**Props（main.vue / ElImage）**

| 属性 | 类型 | 默认值 | 说明 |
|---|---|---|---|
| `src` | String | — | 缩略图展示地址 |
| `fit` | String | — | object-fit 模式，透传给 img |
| `lazy` | Boolean | — | 是否懒加载（进入视口才加载） |
| `scrollContainer` | — | — | 懒加载滚动容器，支持 Element / 选择器字符串 |
| `previewSrcList` | Array | `[]` | 预览大图地址列表，非空时启用预览 |
| `paramsInfo` | Array | `[]` | 每张图的扩展信息（name、ext 等），供下载时构造文件名 |
| `saveUrlList` | Array | `[]` | "记住旋转方向"接口使用的 OSS 原始地址列表；`previewSrcList` 为 blob URL 时必须传此项 |
| `showDownBtn` | Boolean | `false` | 是否显示下载按钮 |
| `zIndex` | Number | `2000` | 预览弹窗 z-index |
| `initialIndex` | Number | — | 点击时默认显示的图片索引（不传则匹配 src） |

**Props（image-viewer.vue，由 main.vue 内部传入，外部不直接操作）**

| 属性 | 类型 | 默认值 | 说明 |
|---|---|---|---|
| `urlList` | Array | `[]` | 预览图片地址列表 |
| `initialIndex` | Number | `0` | 初始显示第几张 |
| `zIndex` | Number | `2000` | 弹窗层级 |
| `onSwitch` | Function | — | 切换图片时回调（index） |
| `onClose` | Function | — | 关闭弹窗回调 |
| `appendToBody` | Boolean | `true` | 是否挂载到 body |
| `maskClosable` | Boolean | `true` | 点击遮罩是否关闭 |
| `showDownBtn` | Boolean | `false` | 是否显示下载按钮 |
| `paramsInfo` | Array | `[]` | 图片扩展信息，用于下载文件名 |
| `saveUrlList` | Array | `[]` | OSS 原始地址列表，用于旋转保存接口 |

**Events（外层 ElImageX 对外暴露）**

| 事件名 | 说明 | 返回值 |
|---|---|---|
| `prev` | 切换到上一张 | 当前 index |
| `next` | 切换到下一张 | 当前 index |
| `onSwitch` | 切换图片（image-viewer 内部 watch index 触发） | 当前 index |
| `update` | 旋转方向保存成功后触发 | `true` |
| `clickHandler` | 点击缩略图时触发 | imageIndex |
| `error` | 图片加载失败 | 原生 error 事件 |

**插槽**

| 插槽名 | 作用域 | 说明 |
|---|---|---|
| `placeholder` | main.vue | 图片加载中时显示，默认灰色占位块 |
| `error` | main.vue | 图片加载失败时显示，默认"加载失败"文字 |
| `slot-html` | image-viewer.vue | 预览弹窗内自定义内容区域（透传穿透） |

**预览弹窗操作能力**

| 操作 | 说明 |
|---|---|
| 缩放 | 工具栏放大/缩小 + 鼠标滚轮（rafThrottle 节流） |
| 旋转 | 左旋/右旋各 90°，维护 `rotate` 状态（0 / 90 / 180 / 270） |
| 拖拽 | 鼠标按住图片拖移位置 |
| 切换显示模式 | CONTAIN（适应容器）↔ ORIGINAL（原始尺寸） |
| 键盘快捷键 | ESC 关闭 / Space 切换模式 / ← 上一张 / → 下一张 / ↑ 放大 / ↓ 缩小 |
| 记住旋转方向 | 调用 `imageRotateSave` 接口，保存后关闭弹窗 |
| 下载 | blob URL：直接 `<a>` 触发下载；普通 URL：axios 请求后转 blob 再下载 |

**下载按钮显示规则**

`showDownBtn` 为 `true` 且当前为第 0 张图，且 `paramsInfo[0].name` 为 `'装货磅单'` 或 `'卸货磅单'` 时才显示下载按钮。

**saveUrlList 使用场景**

当调用方将图片转成 blob URL 传给 `previewSrcList` 时，`imageRotateSave` 接口需要的是 OSS 原始地址。此时必须通过 `saveUrlList` 单独传入原始地址列表，两者下标一一对应。

**API 依赖**

| 接口函数 | 来源 | 用途 |
|---|---|---|
| `imageRotateSave` | `@/api/PlatformUmDriver` | 保存图片旋转方向至后端 |

**使用方（logisticsweb 内）**

| 文件 | 使用场景 |
|---|---|
| `components/Upload/index.vue` | 通用上传组件内的图片预览 |
| `components/UploadLocalFile/index.vue` | 本地文件上传组件内的图片预览 |
| `views/userBase/waybillSettlement/edit.vue` | 运单结算编辑页图片预览 |
| `views/userBase/waybillSettlement/read.vue` | 运单结算查看页图片预览 |
| `views/userBase/waybillSettlement/components/WeighBillCarousel.vue` | 磅单轮播组件图片预览 |
| `views/userBase/waybillSettlement/components/SettlementInfo.vue` | 结算信息组件图片预览 |
| `views/userBase/waybillSettlement/components/CostMessageDetails.vue` | 费用明细组件图片预览 |
| `views/userBase/transportReceiptManagement/index.vue` | 运单回单管理页图片预览 |
| `views/userBase/driverChecking/add.vue` | 司机对账新增页图片预览 |

**全局注册**

在 `src/main.js` 中统一注册，全局可直接使用 `<el-image-x>`，无需局部引入。
...
<el-image-x
  :src="item.blobUrl"
  :preview-src-list="blobUrlList"
  :save-url-list="ossUrlList"
  :params-info="paramsInfo"
  :show-down-btn="true"
  @update="handleRotateSaved"
/>
```

**注意事项**

- `currentImg` 计算属性会在非 blob URL 后追加随机时间戳（`?random=xxx`），防止浏览器缓存导致旋转后仍显示旧版本
- 弹窗挂载到 `document.body`（`appendToBody: true`），销毁时自动移除 DOM
- `image-viewer.vue` 开头有 `/* eslint-disable */`，属于从 Element UI 源码迁移的组件，不受项目 ESLint 规则约束
